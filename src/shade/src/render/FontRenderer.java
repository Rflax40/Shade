...
public class FontRenderer {
	...
	public FontRenderer(IFont f) {
		...
		for (int i = 0; i < count; i++) {
			...
			font.texture(i).drawModalRect(glyph.xCoord, glyph.yCoord, glyph.width, glyph.height, 0,
	...
	public double drawGlyph(int gi, double x, double y, double z, int s) {
		...
		GL11.glPopMatrix();
		// font.texture(gi).drawModalRect(glyph.xCoord, glyph.yCoord,
		// glyph.width, glyph.height, x + ra * glyph.xOffset, y - ra *
		// glyph.yOffset, z, ra * glyph.width, ra * glyph.height);
	...
	public void drawString(String str, double x, double y, double z, int s) {
		...
		for (char c : str.toCharArray()) {
			double i = drawChar(c, drawX, drawY, z, s);
			if (i < 0)
				i = 0;
	...
	public Duple<Double, Double> getBounds(int size, String... strs) {
		double maxW = 0;
		...
		return new Duple<Double, Double>(maxW, height);
	}

	public Duple<Double, Double> getBounds(String... strs) {
